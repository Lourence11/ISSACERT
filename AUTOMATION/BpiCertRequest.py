...
RULES = [
    ...
    # RECOM (OTP)
    {
        ...
        "file_name": "RUSH RECOM FOR CLOSURE & FOR CERT (WOFF - NON LEGAL) - MADRID - RECO 1 (OTP) - {date} - CTRL#{ctrl_no}",
    ...
    },
    {
        ...
        "payment_type": "OTP",
        "levels": ["BPI RECOV 2"],
        "file_name": "RUSH RECOM FOR CLOSURE & FOR CERT (WOFF - NON LEGAL) - MADRID - RECO 2 (OTP) - {date} - CTRL#{ctrl_no}",
    ...
    },
    {
        ...
        "payment_type": "OTP",
        "levels": ["BPI RECOV 3"],
        "file_name": "RUSH RECOM FOR CLOSURE & FOR CERT (WOFF - NON LEGAL) - MADRID - RECO 3 (OTP) - {date} - CTRL#{ctrl_no}",
    ...
    # RECOM (TERM)
    {
        ...
        "payment_type": "TERM",
        "levels": ["BPI RECOV 1", "BPI SPECIAL PROJECT - REVIVAL"],
        "file_name": "RUSH RECOM FOR CLOSURE & FOR CERT (WOFF - NON LEGAL) - MADRID - RECO 1 (TERM) - {date} - CTRL#{ctrl_no}",
    ...
    },
    {
        ...
        "payment_type": "TERM",
        "levels": ["BPI RECOV 2"],
        "file_name": "RUSH RECOM FOR CLOSURE & FOR CERT (WOFF - NON LEGAL) - MADRID - RECO 2 (TERM) - {date} - CTRL#{ctrl_no}",
    ...
    },
    {
        ...
        "payment_type": "TERM",
        "levels": ["BPI RECOV 3"],
        "file_name": "RUSH RECOM FOR CLOSURE & FOR CERT (WOFF - NON LEGAL) - MADRID - RECO 3 (TERM) - {date} - CTRL#{ctrl_no}",
...
def get_quarter_for_date(current_datetime=None):
    current_datetime = current_datetime or datetime.today()
    return ((current_datetime.month - 1) // 3) + 1


def build_output_password(current_datetime=None):
    current_datetime = current_datetime or datetime.today()
    quarter = get_quarter_for_date(current_datetime)
    return f"MAD_{quarter}Q{current_datetime.year}"


def encrypt_workbook_bytes(workbook_bytes, password):
    encrypted_buffer = BytesIO()
    OOXMLFile(BytesIO(workbook_bytes)).encrypt(password, encrypted_buffer)
    encrypted_buffer.seek(0)
    return encrypted_buffer.getvalue()


def build_workbook_buffer(workbook):
    buffer = BytesIO()
...
def build_updated_source_file_name(source_file_name):
    base_name = os.path.basename(source_file_name) if source_file_name else "BPI Cert Request Drive.xlsx"
    stem, _ = os.path.splitext(base_name)
    return ensure_excel_extension(f"{stem} - WITH DA REMARKS")


def prepare_outputs(uploaded_source, source_file_name=None, progress_bar=None):
    ...
    output_password = build_output_password()
    outputs = []
    da_remarks_updates = {}
    ...
    for idx, rule in enumerate(RULES, start=1):
        ...
        if matched_rows:
            ...
            for row in matched_rows:
                row_updates = da_remarks_updates.setdefault(row, [])
                if file_name not in row_updates:
                    row_updates.append(file_name)
            ...
            outputs.append(
                {
                    ...
                    "file_name": file_name,
                    "data": encrypt_workbook_bytes(output_file.getvalue(), output_password),
                }
            )

    for row, file_names in da_remarks_updates.items():
        ws.cell(row=row, column=da_remarks_col).value = " | ".join(file_names)
    ...
    updated_source_buffer = build_workbook_buffer(workbook)
    return {
        "outputs": outputs,
        "updated_source_data": encrypt_workbook_bytes(updated_source_buffer.getvalue(), output_password),
        "updated_source_file_name": build_updated_source_file_name(source_file_name),
        "updated_rows_count": len(da_remarks_updates),
...
def run():
    ...
    if "bpi_cert_updated_source_data" not in st.session_state:
        st.session_state["bpi_cert_updated_source_data"] = None
    if "bpi_cert_updated_source_file_name" not in st.session_state:
        st.session_state["bpi_cert_updated_source_file_name"] = None
    if "bpi_cert_updated_rows_count" not in st.session_state:
        st.session_state["bpi_cert_updated_rows_count"] = 0
    if "bpi_cert_output_password" not in st.session_state:
        st.session_state["bpi_cert_output_password"] = build_output_password()
            ...
            try:
                with st.spinner("Generating files, please wait..."):
                    ...
                    result = prepare_outputs(
                        file_bytes,
                        source_file_name=st.session_state["bpi_cert_uploaded_name"],
                        progress_bar=progress_bar
                    )
                    st.session_state["bpi_cert_generated_outputs"] = result["outputs"]
                    st.session_state["bpi_cert_updated_source_data"] = result["updated_source_data"]
                    st.session_state["bpi_cert_updated_source_file_name"] = result["updated_source_file_name"]
                    st.session_state["bpi_cert_updated_rows_count"] = result["updated_rows_count"]
                    st.session_state["bpi_cert_output_password"] = result["output_password"]
    ...
    updated_source_data = st.session_state["bpi_cert_updated_source_data"]
    updated_source_file_name = st.session_state["bpi_cert_updated_source_file_name"]
    updated_rows_count = st.session_state["bpi_cert_updated_rows_count"]
    output_password = st.session_state["bpi_cert_output_password"]
    ...
    if outputs:
        ...
        st.caption(f'Generated file password: "{output_password}"')
        if updated_source_data:
            st.info(f'DA Remarks updated for {updated_rows_count} row(s) in "{SHEET_NAME}".')
            st.download_button(
                label="Download Updated Source File",
                data=updated_source_data,
                file_name=updated_source_file_name,
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="bpi_cert_download_updated_source",
            )
